...
plic_t::plic_t(const simif_t *sim, uint32_t ndev)
    : num_ids(ndev + 1), num_ids_word(((ndev + 1) + (32 - 1)) / 32),
      max_prio((1UL << PLIC_PRIO_BITS) - 1), priority{}, level{}, pending{},
...
uint32_t plic_t::context_best_pending(const plic_context_t *c) {
  ...
  for (uint32_t i = 0; i < num_ids_word; i++) {
    ...
    for (uint32_t j = 0; j < 32; j++) {
      uint32_t id = i * 32 + j;
      if ((num_ids <= id) || !(pending[i] & (1 << j)) ||
          (claimed[i] & (1 << j)) || !(c->enable[i] & (1 << j))) {
        continue;
      }

      if (!best_id || (best_id_prio < pending_priority[id])) {
        best_id = id;
        best_id_prio = pending_priority[id];
      }
    }
  }
...
uint32_t plic_t::context_claim(plic_context_t *c) {
  ...
  if (best_id) {
    claimed[best_id_word] |= best_id_mask;
    global_update();
  }
...
bool plic_t::context_enable_read(const plic_context_t *c, reg_t offset,
                                 uint32_t *val) {
  uint32_t id_word = offset >> 2;
...
bool plic_t::context_enable_write(plic_context_t *c, reg_t offset,
                                  uint32_t val) {
  uint32_t id_word = offset >> 2;
...
bool plic_t::context_read(plic_context_t *c, reg_t offset, uint32_t *val) {
  switch (offset) {
...
bool plic_t::context_write(plic_context_t *c, reg_t offset, uint32_t val) {
  ...
  case CONTEXT_CLAIM: {
    ...
    if ((val < num_ids) && (c->enable[id_word] & id_mask)) {
...
std::string plic_generate_dts(const sim_t *sim,
  ...
  s << std::hex
    << ">;\n"
       "      reg = <0x"
    << (plicbs >> 32) << " 0x" << (plicbs & (uint32_t)-1) << " 0x"
    << (plicsz >> 32) << " 0x" << (plicsz & (uint32_t)-1)
    << ">;\n"
       "      riscv,ndev = <0x"
    << PLIC_NDEV
    << ">;\n"
       "      riscv,max-priority = <0x"
    << ((1U << PLIC_PRIO_BITS) - 1)
    << ">;\n"
       "      #interrupt-cells = <1>;\n"
