...
#define VIRTIO_NET_F_GUEST_CSUM                                                \
  (1ULL << 1)                        // Guest handles pkts w/ partial csum
...
#define VIRTIO_NET_F_CTRL_MAC_ADDR                                             \
  (1ULL << 23)                       // Set MAC address through control channel
...
#define VIRTIO_NET_HDR_SIZE_MRG                                                \
  12 // Size with num_buffers (when MRG_RXBUF negotiated)
...
struct virtio_net_slirp_config_t {
  ...
  std::vector<std::pair<uint16_t, uint16_t>>
      port_forwards; // host_port -> guest_port
  bool debug;

  virtio_net_slirp_config_t() : enabled(false), debug(false) {}
};
...
public:
  ...
  // Constructor for TAP mode
  virtio_net_t(simif_t *sim, abstract_interrupt_controller_t *intctrl,
...
protected:
  ...
  uint32_t get_config_size() const override {
    return sizeof(virtio_net_config);
  }
...
private:
  ...
  static const size_t MAX_RX_QUEUE_SIZE =
      256; // Limit pending RX packets (match QEMU default)

  // TAP networking
  tap_net_t *tap_net;
  void tap_rx_callback(const uint8_t *data, size_t len);
  void init_tap(const std::string &tap_name);
...
std::string virtio_net_generate_dts(const sim_t *sim,
                                    const std::vector<std::string> &sargs);
...
const std::string &get_virtio_net_tap_name();
bool is_virtio_net_tap_enabled();
