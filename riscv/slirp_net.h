...
struct slirp_config_t {
  ...
  slirp_config_t()
      : net_addr("10.0.2.0"), net_mask("255.255.255.0"), host_addr("10.0.2.2"),
        guest_addr("10.0.2.15"), dns_addr("10.0.2.3"), host_port(8080),
        guest_port(80), debug(false) {}
};
...
public:
  ...
  void set_rx_callback(slirp_rx_callback_t callback) { rx_callback = callback; }

  // Check if initialized
  bool is_initialized() const { return slirp != nullptr; }

  // Set callback for checking if receiver can accept packets (QEMU-style
  // backpressure)
  using can_receive_callback_t = std::function<bool()>;
  void set_can_receive_callback(can_receive_callback_t callback) {
...
private:
  ...
  can_receive_callback_t can_receive_callback;
  ...
  // Notify flag - set when SLIRP wants immediate attention
  bool notify_pending;

public:
  // Check if notify is pending (for caller to poll more aggressively)
  bool has_notify_pending() const { return notify_pending; }
  void clear_notify_pending() { notify_pending = false; }
};
