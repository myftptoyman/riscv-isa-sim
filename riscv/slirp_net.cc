...
slirp_net_t::slirp_net_t(const slirp_config_t &config)
    : config(config), slirp(nullptr), poll_fds(nullptr), poll_fd_count(0),
      poll_fd_capacity(MAX_POLL_FDS), notify_pending(false) {
...
ssize_t slirp_net_t::send_packet_cb(const void *buf, size_t len, void *opaque) {
  ...
  if (debug_slirp) {
    fprintf(stderr,
            "slirp: send_packet_cb len=%zu (packet from network to guest)\n",
            len);
  }
...
void *slirp_net_t::timer_new_cb(SlirpTimerCb cb_func, void *cb_opaque,
                                void *opaque) {
  static bool debug_slirp = getenv("DEBUG_VIRTIO") != nullptr;
  slirp_net_t *self = static_cast<slirp_net_t *>(opaque);
...
void slirp_net_t::notify_cb(void *opaque) {
  // SLIRP is requesting immediate attention - set flag so caller polls more
  // aggressively
  slirp_net_t *self = static_cast<slirp_net_t *>(opaque);
...
int slirp_net_t::add_poll_cb(int fd, int events, void *opaque) {
  ...
  if (self->poll_fd_count < self->poll_fd_capacity) {
    ...
    if (events & SLIRP_POLL_IN)
      self->poll_fds[self->poll_fd_count].events |= POLLIN;
    if (events & SLIRP_POLL_OUT)
      self->poll_fds[self->poll_fd_count].events |= POLLOUT;
...
int slirp_net_t::get_revents_cb(int idx, void *opaque) {
  slirp_net_t *self = static_cast<slirp_net_t *>(opaque);
  if (idx < 0 || idx >= self->poll_fd_count)
    return 0;
  int revents = 0;
  if (self->poll_fds[idx].revents & POLLIN)
    revents |= SLIRP_POLL_IN;
  if (self->poll_fds[idx].revents & POLLOUT)
    revents |= SLIRP_POLL_OUT;
  if (self->poll_fds[idx].revents & POLLERR)
    revents |= SLIRP_POLL_ERR;
  if (self->poll_fds[idx].revents & POLLHUP)
...
bool slirp_net_t::init() {
  ...
  if (debug_slirp) {
    fprintf(stderr,
            "slirp: slirp_new succeeded, slirp=%p, timers created=%zu\n",
            (void *)slirp, timers.size());
  }

  // Add port forwarding
  struct in_addr host_addr_s = {.s_addr = INADDR_ANY};
  struct in_addr guest_addr_s = {.s_addr =
                                     inet_addr(config.guest_addr.c_str())};
  ...
  if (rc < 0) {
    fprintf(stderr,
            "slirp_net: warning - failed to add port forwarding (rc=%d, "
            "errno=%d: %s)\n",
            rc, errno, strerror(errno));
    fprintf(stderr,
            "slirp_net: tried host_port=%d, guest_addr=%s, guest_port=%d\n",
            config.host_port, config.guest_addr.c_str(), config.guest_port);
  } else {
...
void slirp_net_t::send_packet(const uint8_t *data, size_t len) {
  ...
  if (debug_slirp) {
    fprintf(stderr,
            "slirp: send_packet len=%zu (packet from guest to network)\n", len);
  }
  if (slirp) {
...
void slirp_net_t::poll(int timeout_ms) {
  ...
  slirp_pollfds_fill(slirp, &slirp_timeout, add_poll_cb, this);

  // Use non-blocking poll to avoid stalling simulation
  int ret = ::poll(poll_fds, poll_fd_count, 0);
  ...
  // select_error should only be true on actual poll error (ret < 0)
  // When ret == 0 (no events/timeout), we still want SLIRP to process sockets
  // so it can send pending data like TCP FIN packets
