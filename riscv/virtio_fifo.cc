...
void virtio_fifo_t::process_tx_queue() {
  ...
  while (queues[QUEUE_TX].has_pending()) {
    ...
    // Process TX: read data from guest and send to host
    uint32_t total_len = 0;
    for (auto& desc : out_descs) {
      std::vector<uint8_t> buf(desc.len);
      if (queues[QUEUE_TX].read_guest_mem(desc.addr, buf.data(), desc.len)) {
        ssize_t sent = send_data(buf.data(), desc.len);
        if (sent > 0) {
          total_len += sent;
        }
      }
                                          ...
                                          const std::vector<std::string>& sargs)
{
  ...
  // Only create device if socket path is configured
